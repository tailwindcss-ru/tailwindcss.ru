...
Чтобы узнать больше, прочтите документацию Tailwind CSS по [упорядочению составных модификаторов](/docs/hover-focus-and-other-states#ordering-stacked-modifiers).
...
Настройки следует применять к определенному модификатору, например `DEFAULT` или `xl`, и добавлять их в свойство `css`. Настройки создаются с использованием того же [синтаксиса CSS-in-JS](/docs/plugins#css-in-js-syntax), который используется для написания плагинов Tailwind.
