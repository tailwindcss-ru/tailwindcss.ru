...
// При загрузке страницы или смене темы лучше добавить в строку в `head`, чтобы избежать FOUC
...
При использовании этой стратегии Tailwind никак не изменяет предоставленный селектор, поэтому помните о его специфичности и рассмотрите возможность использования псевдокласса `:where()`, чтобы гарантировать, что он имеет ту же специфичность, что и другие утилиты.
