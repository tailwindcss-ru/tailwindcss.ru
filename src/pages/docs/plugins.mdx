...
Плагин `@tailwindcss/aspect-ratio` является альтернативой встроенной поддержке `aspect-ratio`, которая работает в старых браузерах, и добавляет классы `aspect-w-{n}` и `aspect-h-{n}`, которые могут быть объединены, чтобы дать элементу фиксированное соотношение сторон.
