---
title: "Отступы прокрутки"
description: "Утилиты для управления смещением прокрутки элемента в контейнере привязки."
---
...
Используйте утилиты `scroll-pt-*`, `scroll-pr-*`, `scroll-pb-*` и `scroll-pl-*`, чтобы установить смещение прокрутки элемента внутри контейнера привязки.
...
Используйте утилиты `scroll-ps-*` и `scroll-pe-*`, чтобы установить `scroll-padding-inline-start` и `scroll-padding-inline-end` [логические свойства](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_Logical_Properties/Basic_concepts), которые сопоставляются с левой или правой стороной в зависимости от направления текста.
...
По умолчанию шкала прокрутки Tailwind использует [шкалу интервалов по умолчанию](/docs/customizing-spacing). Вы можете настроить масштаб интервалов, отредактировав `theme.spacing` или `theme.extend.spacing` в вашем файле `tailwind.config.js`.
