...
Например, в шаблоне ниже вы можете увидеть, что служебные классы для каждого изображения аватара повторяются шесть отдельных раз:

<Example p="none">
  <div class="w-72 sm:w-96 px-8 sm:px-12 py-6 sm:py-8 mx-auto bg-white shadow">
    <div class="flex text-base items-center space-x-2">
      <h4 class="text-base font-semibold text-gray-900">Контрибьюторы</h4>
...
<div>
  <div class="flex items-center space-x-2 text-base">
    <h4 class="font-semibold text-gray-900">Контрибьюторы</h4>
...
Такое дублирование в большинстве случаев даже не является реальной проблемой, потому что оно все собрано в одном месте, или даже не существует на самом деле, потому что вы выполняете итерацию по массиву элементов и пишете разметку только один раз.
...
<Example p="none">
  <div class="w-72 sm:w-96 px-8 sm:px-12 py-6 sm:py-8 mx-auto bg-white shadow">
    <div>
      <img class="rounded" src="https://images.unsplash.com/photo-1452784444945-3f422708fe5e?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=512&q=80" width="512" height="341" alt="Пляж" />
      <div class="mt-2">
        <div>
          <div class="text-xs text-gray-600 uppercase font-bold tracking-wider">Частная вилла</div>
          <div class="font-bold text-gray-700 leading-snug">
            <a href="#" class="hover:underline">Расслабляющий курорт "все включено" в Канкуне</a>
          </div>
          <div class="mt-2 text-sm text-gray-600">$299 долларов за ночь</div>
        </div>
      </div>
    </div>
  </div>
  ...
  <div class="px-6 py-12">
    <div class="max-w-sm mx-auto p-6 flex items-center bg-white rounded-xl shadow-md space-x-4">
      ...
      <div>
        ...
        <p class="text-sm sm:text-base text-gray-500">У вас есть новое сообщение!</p>
      </div>
    </div>
  </div>
</Example>
...
```html HTML
<!-- Перед извлечением настраиваемого класса -->
...
### Как избежать преждевременного абстрагирования

Что бы вы ни делали, **не используйте <code className="font-semibold text-gray-900">@apply</code> только для того, чтобы все выглядело «чище»**. Да, HTML-шаблоны, заваленные классами Tailwind, некрасивы. Вносить изменения в проект, в котором много кастомных CSS, хуже.
