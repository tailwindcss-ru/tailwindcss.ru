...
Если вы используете вложение в своем пользовательском CSS (в сочетании с плагином вложения PostCSS), вам также следует [настроить плагин `tailwindcss/nesting`](https://tailwindcss.com/docs/using-with-preprocessors#nesting) в вашей конфигурации PostCSS, чтобы обеспечить совместимость с Tailwind CSS v3.0.
...
Поскольку все варианты теперь включены по умолчанию, вам больше не нужно явно включать их для настраиваемого CSS с помощью директив `@variants` или `@responsive`.
...
Утилиты `fill-{color}` и `stroke-{color}` теперь отражают ваш ключ `theme.colors` по умолчанию. Это не является критическим изменением, если вы не настроили свою цветовую палитру, но если вы это сделали, классы `fill-current` и `stroke-current` могут не работать, если вы не включили `current` в свою цветовую палитру. собственная цветовая палитра.
...
Отрицательный префикс в таких утилитах, как `-mx-4`, теперь является первоклассной функцией в Tailwind, а не чем-то, обусловленным вашей темой, поэтому вы можете добавить `-` перед любой утилитой, которая поддерживает отрицательные значения, и она будет работать.
