...
<Example p="none">
  <div class="mx-auto max-w-xs bg-white shadow-xl p-4 text-slate-500 text-sm leading-6 sm:text-base sm:leading-7 dark:bg-slate-800 dark:text-slate-400">
    ...
    поместит элемент в отдельную строку и заполнит его родителя.
  </div>
</Example>
...
Используйте `hidden`, чтобы установить для элемента значение `display: none` и удалить его из макета страницы (сравните с `invisible` из документации [visibility](/docs/visibility#invisible)).
