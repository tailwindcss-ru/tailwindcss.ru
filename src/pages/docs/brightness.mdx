...
### Изменение яркости элемента

Используйте утилиты `brightness-{amount?}` для управления яркостью элемента.
...
По умолчанию Tailwind включает в себя несколько утилит общего назначения `brightness`. Вы можете настроить эти значения, отредактировав `theme.brightness` или `theme.extend.brightness` в вашем файле `tailwind.config.js`.
