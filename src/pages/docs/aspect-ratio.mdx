...
Утилиты `aspect-{ratio}` используют собственное свойство CSS `aspect-ratio`, которое не поддерживалось в Safari до версии 15. До тех пор, пока не станет популярным Safari 15, плагин Tailwind [соотношение сторон](https://github.com/tailwindlabs/tailwindcss-aspect-ratio) является хорошей альтернативой.
