...
- Утилиты Tailwind [flexbox](/docs/display#flex) и [padding](/docs/padding) для управления общим макетом карты (`flex`, `shrink-0` и `p-6`)
- Утилиты [max-width](/docs/max-width) и [margin](/docs/margin) для ограничения ширины карты и ее центрирования по горизонтали (`max-w-sm` и `mx-auto`)
- Утилиты [background color](/docs/background-color), [border radius](/docs/border-radius) и [box-shadow](/docs/box-shadow) для стилизации внешнего вида карты (`bg-white`, `rounded-xl` и `shadow-lg`)
- Утилиты [size](/docs/size) для установки ширины и высоты изображения логотипа (`size-12`)
- Утилиты [gap](/docs/gap) для управления интервалом между логотипом и текстом (`gap-x-4`)
- Утилиты [font size](/docs/font-size), [text color](/docs/text-color) и [font-weight](/docs/font-weight) для стилизации текста карточки (`text-xl`, `text-black`, `font-medium` и т. д.)
