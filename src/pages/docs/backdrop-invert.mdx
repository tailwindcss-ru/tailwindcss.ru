...
### Инвертирование фона элемента

Используйте утилиты `backdrop-invert` и `backdrop-invert-0` для управления тем, должен ли элемент отображаться с инвертированными цветами фона или в обычном режиме.
...
По умолчанию Tailwind включает в себя несколько утилит общего назначения `backdrop-invert`. Вы можете настроить эти значения, отредактировав `theme.backdropInvert` или `theme.extend.backdropInvert` в вашем файле `tailwind.config.js`.
