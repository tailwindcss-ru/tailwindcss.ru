...
Тема Tailwind по умолчанию настраивает разумную `line-height` по умолчанию для каждой утилиты  `text-{size}`. Вы можете настроить свою собственную высоту строки по умолчанию при использовании пользовательских размеров шрифта, определив каждый размер с помощью кортежа формы `[fontSize, lineHeight]` в вашем файле `tailwind.config.js`.
