...
export const description = "Утилиты для установки ширины элемента.";
...
Используйте утилиты `w-<number>` такие как `w-24` и `w-64` для установки элемента на фиксированную ширину на основе шкалы отступов:
...
Используйте утилиты `w-full` или `w-<fraction>` такие как `w-1/2` и `w-2/5` для установки элемента на процентную ширину:
...
### Использование шкалы контейнера

Используйте утилиты такие как `w-sm` и `w-xl` для установки элемента на фиксированную ширину на основе шкалы контейнера:
...
### Соответствие вьюпорту

Используйте утилиту `w-screen` для растягивания элемента на всю ширину вьюпорта:
...
Альтернативно, вы можете соответствовать ширине большого, малого или динамического вьюпорта, используя утилиты `w-lvw`, `w-svw` и `w-dvw`.

### Сброс ширины

Используйте утилиту `w-auto` для удаления назначенной ширины элемента при определенном условии, например, на определенном брейкпоинте:
...
Используйте утилиты такие как `size-px`, `size-4` и `size-full` для одновременной установки ширины и высоты элемента:
