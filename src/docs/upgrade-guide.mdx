...
Наш [инструмент обновления](#using-the-upgrade-tool) автоматически обработает большинство этих изменений, поэтому мы настоятельно рекомендуем использовать его, если вы можете.
...
Утилита `outline` теперь устанавливает `outline-width: 1px` по умолчанию, чтобы быть более согласованной с утилитами border и ring. Кроме того, все утилиты `outline-<number>` по умолчанию устанавливают `outline-style` на `solid`, исключая необходимость объединять их с `outline`:
...
Ранее утилита `outline-none` фактически не устанавливала `outline-style: none`, а вместо этого устанавливала невидимый контур, который по-прежнему отображался в режиме принудительной настройки цветов из соображений доступности.
...
Вы можете найти больше документации по [использованию Tailwind с модулями CSS](/docs/compatibility#css-modules).

### Использование Sass, Less и Stylus
...
Узнайте больше в [документации по совместимости](/docs/compatibility#sass-less-and-stylus).
