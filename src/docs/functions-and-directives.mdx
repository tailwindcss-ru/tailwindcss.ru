...
Параметры `corePlugins`, `safelist` и `separator` из конфигурации на основе JavaScript не поддерживаются в v4.0. Для внесения утилит в безопасный список в v4 используйте [`@source inline()`](/docs/detecting-classes-in-source-files#safelisting-specific-utilities).
