...
export const description = "Утилиты для управления цветом границ элемента.";
...
Используйте утилиты вроде `border-rose-500` и `border-lime-100` для управления цветом границы элемента:
...
Используйте модификатор прозрачности цвета для управления прозрачностью цвета границы элемента:
...
Используйте утилиты вроде `border-t-indigo-500` и `border-r-lime-100` для установки цвета границы одной стороны элемента:
...
Используйте утилиты вроде `border-x-indigo-500` и `border-y-lime-100` для одновременной установки цвета границы на двух сторонах элемента:
...
Используйте утилиты вроде `border-s-indigo-500` и `border-e-lime-100` для установки [логических свойств](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_Logical_Properties/Basic_concepts) `border-inline-start-color` и `border-inline-end-color`, которые соответствуют левой или правой границе в зависимости от направления текста:
...
Используйте утилиты вроде `divide-indigo-500` и `divide-lime-100` для управления цветом границы между дочерними элементами:
