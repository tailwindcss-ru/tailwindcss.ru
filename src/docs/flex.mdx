...
export const description = "Утилиты для управления тем, как flex элементы растут и сжимаются.";
...
Используйте утилиты `flex-<number>` типа `flex-1` для разрешения flex элементу расти и сжиматься по мере необходимости, игнорируя его начальный размер:
...
Используйте `flex-initial` для разрешения flex элементу сжиматься, но не расти, учитывая его начальный размер:
...
Используйте `flex-auto` для разрешения flex элементу расти и сжиматься, учитывая его начальный размер:
...
Используйте `flex-none` для предотвращения роста или сжатия flex элемента:
