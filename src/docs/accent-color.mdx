import { ApiTable } from "@/components/api-table.tsx";
import {
  CustomizingYourThemeColors,
  ResponsiveDesign,
  TargetingSpecificStates,
  UsingACustomValue,
} from "@/components/content.tsx";
import { Example } from "@/components/example.tsx";
import { Figure } from "@/components/figure.tsx";
import colors from "./utils/colors";

export const title = "accent-color";
export const description = "Утилиты для управления акцентным цветом элемента формы.";

<ApiTable
  rows={[
    ["accent-inherit", "accent-color: inherit;"],
    ["accent-current", "accent-color: currentColor;"],
    ["accent-transparent", "accent-color: transparent;"],
    ...Object.entries(colors).map(([name, value]) => [
      `accent-${name}`,
      `accent-color: var(--color-${name}); /* ${value} */`,
    ]),
    ["accent-<custom-property>", "accent-color: var(<custom-property>);"],
    ["accent-[<value>]", "accent-color: <value>;"],
  ]}
/>

## Примеры

### Установка акцентного цвета

Используйте утилиты, такие как `accent-rose-500` и `accent-lime-600`, чтобы изменить акцентный цвет элемента:

<Figure>

<Example>
  {
    <div className="flex flex-wrap justify-center gap-6">
      <label className="flex items-center space-x-2">
        <input type="checkbox" defaultChecked />
        <div className="text-sm font-semibold text-gray-900 dark:text-gray-200">Браузер по умолчанию</div>
      </label>
      <label className="flex items-center space-x-2">
        <input type="checkbox" className="accent-pink-500" defaultChecked />
        <div className="text-sm font-semibold text-gray-900 dark:text-gray-200">Кастомизированный</div>
      </label>
    </div>
  }
</Example>

```html
<!-- [!code classes:accent-pink-500] -->
<label>
  <input type="checkbox" checked />
  Браузер по умолчанию
</label>
<label>
  <input class="accent-pink-500" type="checkbox" checked />
  Кастомизированный
</label>
```

</Figure>

Это полезно для стилизации элементов, таких как чекбоксы и радиогруппы, позволяя переопределить стандартный цвет браузера.

### Изменение прозрачности

Используйте модификатор прозрачности цвета, чтобы управлять прозрачностью акцентного цвета элемента:

<Figure>

<Example>
  {
    <div className="flex flex-wrap justify-center gap-6">
      <label className="flex items-center space-x-2">
        <input type="checkbox" defaultChecked className="accent-purple-500/25" />
        <div className="text-sm font-semibold text-gray-900 dark:text-gray-200">accent-purple-500/25</div>
      </label>
      <label className="flex items-center space-x-2">
        <input type="checkbox" className="accent-purple-500/75" defaultChecked />
        <div className="text-sm font-semibold text-gray-900 dark:text-gray-200">accent-purple-500/75</div>
      </label>
    </div>
  }
</Example>

```html
<!-- [!code word:/25] -->
<!-- [!code word:/75] -->
<input class="accent-purple-500/25" type="checkbox" checked />
<input class="accent-purple-500/75" type="checkbox" checked />
```

</Figure>

Установка прозрачности акцентного цвета имеет ограниченную поддержку браузеров и на данный момент работает только в Firefox.

### Использование пользовательского значения

<UsingACustomValue
  element="input"
  elementAttributes={{ type: "checkbox" }}
  utility="accent"
  value="#50d71e"
  name="accent color"
  variable="accent-color"
/>

### Применение при наведении

<TargetingSpecificStates property="accent-color">

<Figure>

<Example>
  {
    <div className="flex flex-wrap justify-center gap-6">
      <label className="flex items-center space-x-2">
        <input type="checkbox" className="accent-black hover:accent-pink-500" defaultChecked />
        <div className="text-sm font-semibold text-gray-900 dark:text-gray-200">Согласиться с условиями</div>
      </label>
    </div>
  }
</Example>

```html
<!-- [!code classes:hover:accent-pink-500] -->
<input class="accent-black hover:accent-pink-500" type="checkbox" />
```

</Figure>

</TargetingSpecificStates>

### Адаптивный дизайн

<ResponsiveDesign
  element="input"
  elementAttributes={{ type: "checkbox" }}
  property="accent-color"
  defaultClass="accent-black"
  featuredClass="accent-pink-500"
/>

## Кастомизация вашей темы

<CustomizingYourThemeColors element="input" elementAttributes={{ type: "checkbox" }} utility="accent" />
