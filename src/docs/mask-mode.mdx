...
export const description = "Утилиты для управления режимом маски элемента.";
...
Используйте утилиты `mask-alpha`, `mask-luminance` и `mask-match` для управления режимом маски элемента:
...
При использовании `mask-luminance` значение яркости маски определяет видимость, поэтому использование оттенков серого даст наиболее предсказуемые результаты. С `mask-alpha` прозрачность маски определяет видимость замаскированного элемента.
