...
export const description = "Утилиты для управления тем, как элементы размеряются и размещаются по строкам сетки.";
...
Используйте утилиты `row-span-<number>` типа `row-span-2` и `row-span-4` для того, чтобы элемент растягивался на _n_ строк:
...
Используйте утилиты `row-start-<number>` или `row-end-<number>` типа `row-start-2` и `row-end-3` для того, чтобы элемент начинался или заканчивался на _n_-й линии сетки:
...
Их также можно комбинировать с утилитами `row-span-<number>` для растягивания на определенное количество строк.
