...
export const description = "Утилиты для указания строк в макете сетки.";
...
### Указание строк сетки

Используйте утилиты `grid-rows-<number>` типа `grid-rows-2` и `grid-rows-4` для создания сеток с _n_ строками одинакового размера:
...
Используйте утилиту `grid-rows-subgrid` для принятия треков строк, определенных родителем элемента:
