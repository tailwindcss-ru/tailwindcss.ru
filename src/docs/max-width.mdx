...
export const description = "Утилиты для установки максимальной ширины элемента.";
...
Используйте утилиты `max-w-<number>` такие как `max-w-24` и `max-w-64` для установки элемента на фиксированную максимальную ширину на основе шкалы отступов:

<Figure hint="Измените размер примера, чтобы увидеть ожидаемое поведение">
...
Используйте утилиты `max-w-full` или `max-w-<fraction>` такие как `max-w-1/2` и `max-w-2/5` для установки элемента на максимальную ширину на основе процентов:
...
### Использование шкалы контейнеров

Используйте утилиты такие как `max-w-sm` и `max-w-xl` для установки элемента на фиксированную максимальную ширину на основе шкалы контейнеров:
...
<Example resizable>
  {
    <div className="dark:highlight-white/5 relative mx-auto flex max-w-md items-center gap-6 overflow-hidden rounded-xl bg-white shadow-lg ring-1 ring-black/5 dark:bg-gray-800">
      ...
      <div className="min-w-0 py-5 pr-5 pl-28">
        <div className="truncate text-sm font-medium text-gray-900 sm:text-base dark:text-gray-200">Эндрю Альфред</div>
        <div className="truncate text-sm leading-tight text-gray-500 sm:text-base dark:text-gray-400">
          Помощник путешествующего секретаря
        </div>
      </div>
    </div>
  }
...
### Использование контейнера с брейкпоинтами

Используйте утилиту `container` для установки максимальной ширины элемента в соответствии с `min-width` текущего брейкпоинта. Это полезно, если вы предпочитаете проектировать для фиксированного набора размеров экранов вместо попытки приспособиться к полностью адаптивному viewport:
...
Обратите внимание, что в отличие от контейнеров, которые вы могли использовать в других фреймворках, контейнер Tailwind не центрируется автоматически и не имеет встроенных горизонтальных отступов. Используйте `mx-auto` и утилиты `px-<number>` для добавления этих свойств:
