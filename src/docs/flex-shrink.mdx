...
export const description = "Утилиты для управления тем, как flex элементы сжимаются.";
...
### Разрешение flex элементам сжиматься

Используйте `shrink` для разрешения flex элементу сжиматься при необходимости:
...
### Предотвращение сжатия элементов

Используйте `shrink-0` для предотвращения сжатия flex элемента:
