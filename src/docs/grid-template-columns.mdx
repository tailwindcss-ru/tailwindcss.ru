...
export const description = "Утилиты для указания столбцов в макете сетки.";
...
### Указание столбцов сетки

Используйте утилиты `grid-cols-<number>` типа `grid-cols-2` и `grid-cols-4` для создания сеток с _n_ столбцами одинакового размера:
...
Используйте утилиту `grid-cols-subgrid` для принятия треков столбцов, определенных родителем элемента:
