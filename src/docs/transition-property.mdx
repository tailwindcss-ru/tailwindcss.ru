...
export const description = "Утилиты для управления тем, какие CSS-свойства анимируются при переходах.";
...
Используйте утилиты типа `transition` и `transition-colors`, чтобы указать, какие свойства должны анимироваться при их изменении:

<Figure hint="Наведите курсор на кнопку, чтобы увидеть ожидаемое поведение">
...
### Поддержка режима уменьшенного движения

В ситуациях, когда пользователь указал, что предпочитает уменьшенное движение, вы можете условно применять анимации и переходы с помощью вариантов `motion-safe` и `motion-reduce`:
