...
export const description = "Утилиты для применения фильтров падающей тени к элементу.";
...
Используйте такие утилиты, как `drop-shadow-sm` и `drop-shadow-xl`, чтобы добавить тень к элементу:
...
Это полезно для применения теней к нерегулярным формам, таким как текст и элементы SVG. Для применения теней к обычным элементам вы, вероятно, захотите использовать [box shadow](/docs/box-shadow) вместо этого.

### Изменение непрозрачности

Используйте модификатор непрозрачности, чтобы настроить непрозрачность падающей тени:
...
Непрозрачность тени по умолчанию довольно низкая (15% или меньше), поэтому увеличение непрозрачности (до 50%) сделает тени более выраженными.

### Установка цвета тени

Используйте утилиты, такие как `drop-shadow-indigo-500` и ​​`drop-shadow-cyan-500/50`, чтобы изменить цвет тени:
...
По умолчанию цветные тени имеют непрозрачность 100%, но вы можете настроить ее с помощью модификатора непрозрачности.

### Удаление тени

Используйте утилиту `drop-shadow-none`, чтобы удалить существующую тень от элемента:
