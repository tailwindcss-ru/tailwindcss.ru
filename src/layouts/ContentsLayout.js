...
function TableOfContents({ tableOfContents, currentSection }) {
  ...
  return (
    <>
      <h5 className="text-gray-900 font-semibold mb-4 text-sm leading-6">На этой странице</h5>
...
export function ContentsLayout({ children, meta, classes, tableOfContents, section }) {
  ...
  const toc = [
    ...(classes ? [{ title: 'Краткая справка', slug: 'class-reference', children: [] }] : []),
    ...tableOfContents,
  ]
  ...
  return (
    <div className="max-w-3xl mx-auto pt-10 xl:max-w-none xl:ml-0 xl:mr-[15.5rem] xl:pr-16">
      <PageHeader
        ...
        badge={{ key: 'Версия Tailwind CSS', value: meta.featureVersion }}
        section={section}
      />
      <ContentsContext.Provider value={{ registerHeading, unregisterHeading }}>
        {classes ? (
      ...
      <Footer previous={prev} next={next}>
        <Link
          href={`https://github.com/tailwindcss-ru/tailwindcss.ru/edit/master-ru/src/pages${router.pathname}.mdx`}
        >
          <a className="hover:text-gray-900">Редактировать эту страницу на GitHub</a>
        </Link>
      </Footer>
